...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
    ...
    local __connectTimeTick = function ()
        ...
        if self.waitConnect >= SOCKET_CONNECT_FAIL_TIMEOUT then
            ...
            print("SocketTCP:close() self.waitConnect:",self.waitConnect,",SOCKET_CONNECT_FAIL_TIMEOUT:",SOCKET_CONNECT_FAIL_TIMEOUT)
            self:close()
        end
        __checkConnect()
    end
...
function SocketTCP:close( ... )
    printInfo("SocketTCP:close( ... ):%s.close", self.name)
    self.tcp:close();
