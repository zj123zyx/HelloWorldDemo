...
function CCBReaderLoad(strFilePath,owner)
    ...
    for i = 1 , table.getn(nodesWithAnimationManagers) do
        ...
        for i = 1,table.getn(documentCallbackNames) do
            ...
            if nil ~= owner then
                if "function" == type(owner[callbackName]) then
                    proxy:setCallback(callbackNode, owner[callbackName], documentCallbackControlEvents[i])
                else
        ...
        for i = 1, table.getn(documentOutletNames) do
            ...
            if nil ~= owner then
                if proxy:getNodeTypeName(outletNode) == "No Support" then
                    if nil ~= outletNode then
                        owner[outletName] = outletNode
                    else
                        print("Warning: the variable %s is wrong",outletName:getCString())
                    end
                else
                    owner[outletName] = tolua.cast(outletNode, proxy:getNodeTypeName(outletNode))
                end
            end
        end
        ...
        --[[
        if nil ~= owner  and type(owner.onDidLoadFromCCB) == "function" then
        ...
        for i = 1 , table.getn(keyframeCallbacks) do
            ...
            if 1 == callbackType and nil ~= owner then
...
local function CCBuilderReaderLoad(strFilePath,owner)
    --print("\n********** \n".."CCBuilderReaderLoad(strFilePath,proxy,owner)".." was deprecated please use ".. --"CCBReaderLoad(strFilePath,owner)" .. " instead.\n**********")
    return CCBReaderLoad(strFilePath,owner)
end
